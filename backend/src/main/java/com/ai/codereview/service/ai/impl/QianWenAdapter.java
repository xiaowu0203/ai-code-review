...
public class QianWenAdapter implements AIModelAdapter {
    ...
    public String reviewCode(String diff, AIModelConfig modelConfig) {
        try {
            ...
            requestBody.put("input", new HashMap<String, String>() {{
                put("messages", buildMessages(prompt));
            }});
            requestBody.put("parameters", new HashMap<String, Object>() {{
                put("max_tokens", modelConfig.getMaxTokens());
                put("temperature", modelConfig.getTemperature());
            }});
            ...
            try (Response response = client.newCall(request).execute()) {
                ...
                throw new RuntimeException("千问API返回格式异常");
            }
        } catch (Exception e) {
            log.error("千问模型审核代码失败", e);
            throw new RuntimeException("千问模型审核失败: " + e.getMessage());
        }
    }
    ...
    public boolean testConnection(AIModelConfig modelConfig) {
        try {
            ...
            requestBody.put("model", "qwen-plus");
            requestBody.put("input", new HashMap<String, String>() {{
                put("messages", "[{\"role\":\"user\",\"content\":\"test\"}]");
            }});
            requestBody.put("parameters", new HashMap<String, Object>() {{
                put("max_tokens", 10);
            }});
     ...
     * 构建消息列表
     */
    private String buildMessages(String prompt) {
        Map<String, Object> message = new HashMap<>();
        message.put("role", "user");
        message.put("content", prompt);
        return JSON.toJSONString(new Object[]{message});
    }
}