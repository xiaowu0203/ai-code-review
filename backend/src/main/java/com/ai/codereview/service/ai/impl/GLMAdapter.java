...
@Slf4j
@Component
public class GLMAdapter implements AIModelAdapter {
    ...
    public String reviewCode(String diff, AIModelConfig modelConfig) {
        try {
            ...
            try (Response response = client.newCall(request).execute()) {
                ...
                throw new RuntimeException("GLM API返回格式异常");
            }
        } catch (Exception e) {
            log.error("GLM模型审核代码失败", e);
            throw new RuntimeException("GLM模型审核失败: " + e.getMessage());
        }
    }
